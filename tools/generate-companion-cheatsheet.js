...
lines.push("- Use `/set?slider=SLIDER_ID&value=VALUE` for a validated absolute Develop value.");
lines.push("- Rapid pending `/set` and `/reset` requests coalesce independently per slider; relative `/adjust` commands retain their existing semantics.");
lines.push("- `/get` and `/last-result` remain experimental and are not the Web Controller feedback path.");
lines.push("");
lines.push("---");
lines.push("");
...
for (const [label, pathPart] of [
    ...
    ["Original Aspect", "/command?command=photo.crop_aspect&mode=original"],
    ["Camera Crop", "/command?command=photo.crop_aspect&mode=asshot"],
    ["1:1", "/command?command=photo.crop_aspect&mode=1x1"],
    ["2:3", "/command?command=photo.crop_aspect&mode=2x3"],
    ["4:5", "/command?command=photo.crop_aspect&mode=4x5"],
    ["5:7", "/command?command=photo.crop_aspect&mode=5x7"],
    ["16:9", "/command?command=photo.crop_aspect&mode=16x9"],
    ["16:10", "/command?command=photo.crop_aspect&mode=16x10"],
    ["Custom Crop 16:10", "/command?command=photo.crop_aspect&mode=custom&w=16&h=10"],
    ["Set Crop Angle -2.5°", "/command?command=photo.crop_angle.set&value=-2.5"],
    ["Reset Crop Angle", "/command?command=photo.crop_angle.reset"],
    ["Show in Explorer", "/command?command=photo.reveal&scope=active"]
]) {
    lines.push(`| ${label} | \`${pathPart}\` | \`${fullUrl(pathPart)}\` |`);
}
lines.push("");
lines.push("Custom Crop accepts whole-number `w` and `h` values from 1 to 10000 and preserves the supplied ratio. It uses Lightroom's documented SDK table form; it does not open Lightroom's native Enter Custom dialog.");
lines.push("");
lines.push("Crop Angle uses the documented `straightenAngle` Develop parameter from -45° to +45°. Rapid pending updates coalesce to the latest state. Reset Angle resets only straightening; Reset Crop resets the complete crop state. Controlled Lightroom runtime verification passed.");
...
lines.push("");
lines.push("The Web Controller and HTTP Builder read each slider's absolute range, interaction step, numeric precision, and feedback support from `config/sliders.json`. Example absolute command:");
lines.push("");
lines.push("```text");
lines.push("/set?slider=Exposure&value=1.25");
lines.push("```");
lines.push("");
lines.push("The Web Controller displays authoritative Lightroom polling feedback. A parameter that is unavailable for the active photo or process version displays `--`; LRBridge does not invent a zero value.");
lines.push("");
...
lines.push("---");
lines.push("");
lines.push("## Experimental result endpoints");
lines.push("");
lines.push("These result-slot endpoints remain experimental:");
lines.push("");
lines.push("```text");
...
lines.push("The Web Controller uses the dedicated authoritative feedback polling routes instead.");
lines.push("```");
lines.push("");
lines.push("---");
lines.push("");
...
lines.push("3. Use `/set` for absolute values, `/adjust` for relative encoders, and `/reset` for one parameter.");
lines.push("4. Do not build feedback on `/last-result`.");
lines.push("5. Derive absolute ranges and precision from slider metadata rather than duplicating tables.");
