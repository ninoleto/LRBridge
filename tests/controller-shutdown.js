...
function testSourceIntegration() {
    ...
    assert.match(main, /require\("\.\/desktop-shutdown"\)/);
    assert.match(main, /createDesktopShutdownCoordinator\(\{/);
    ...
    assert.match(beforeQuit, /isQuitting = true;[\s\S]*desktopShutdownCoordinator\.beforeQuit\(event\)/);
    assert.doesNotMatch(beforeQuit, /controllerServer = null|stopControllerServer\(|app\.quit\(/);
    const lifecycle = fs.readFileSync(path.join(root, "app", "desktop-shutdown.js"), "utf8");
    const coordinatorSource = lifecycle.slice(lifecycle.indexOf("function createDesktopShutdownCoordinator"));
    assert.match(coordinatorSource, /event\.preventDefault\(\)/);
    assert.match(coordinatorSource, /Promise\.all\(operations\)[\s\S]*allowQuit = true;[\s\S]*options\.resumeQuit\(\)/);
    assert.equal((coordinatorSource.match(/options\.resumeQuit\(\)/g) || []).length, 1);
