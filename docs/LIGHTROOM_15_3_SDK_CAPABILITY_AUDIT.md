...
This document records an audit of the official Lightroom Classic 15.3 SDK reference, guides, release notes and sample plug-ins, plus explicitly authorized controlled runtime verification for potential LRBridge commands.
...
Lightroom Classic 15.3 documents the Develop Controller parameters `LensBlurActive`, `LensBlurAmount`, `LensBlurCatEye`, `LensBlurHighlightsBoost`, and `LensBlurFocalRange`, together with `getSelectedLensBlurBokeh()`, `setLensBlurBokeh(value)`, `toggleLensBlurDepthVisualization()`, `selectTool("depth_refinement")`, `getSelectedTool()`, and `revealPanel("lensBlurPanel")`.

The first read-only run captured an important failure state rather than a general API limitation. The Process Version 6 photo appeared to have Lens Blur enabled, but Lightroom's Lens Blur subsystem was not healthy: all five `getValue()` calls and `getSelectedLensBlurBokeh()` returned nil. `getRange()` still returned `-100..100` for `LensBlurActive` and `0..100` for `LensBlurAmount`, `LensBlurCatEye`, and `LensBlurHighlightsBoost`. The asynchronous diagnostic found `photo:getDevelopSettings().LensBlur` as an empty table with zero top-level keys and `DepthMapInfo` as nil. A separate attempt to wrap the yielding `photo:getDevelopSettings()` call in ordinary `pcall` failed with the exact error `Yielding is not allowed within a C or metamethod call`; that diagnostic error is not evidence about either Develop-settings member.
...
After Lightroom was manually restarted, the same disposable photo reached a healthy, manually enabled state and completed its depth-map generation. Lightroom visibly showed Amount 50, Cat Eye 0, Boost 50, and Circle Bokeh. In that state the three scalar getters returned numbers, their ranges were `0..100`, and one controlled `setValue()` call per scalar succeeded and was visually confirmed: Amount 37, Cat Eye 23, and Boost 41. Nil therefore means unavailable; it is not zero, a default, or permission to reuse a prior value.

`LensBlurActive` is **WRITE-ONLY / INTERMITTENT READBACK**. Controlled calls to `setValue("LensBlurActive", false)` and `setValue("LensBlurActive", true)` visibly unchecked and checked Apply. Re-enabling completed depth generation without an AI error. The getter can authoritatively return boolean true or false in a healthy state, but it can also return nil while Apply appears enabled. The asynchronous Develop-settings view is not a reliable substitute: `LensBlur.Active` and `DepthMapInfo` can be absent or stale around the same transition. LRBridge therefore exposes explicit Off and On commands, highlights a selection only from a current boolean getter result, and shows unknown with neither selection when the getter is nil. Command acceptance is never retained as authoritative state.

Lightroom-native `resetToDefault(param)` was also tested independently for all three scalar controls. Setting and verifying Amount 37, Boost 41, and Cat Eye 23 before reset produced authoritative post-reset values 50, 50, and 0 respectively, and each final slider position was visually confirmed. Those numbers are observed Lightroom 15.3 defaults for the test photo; production reset behavior calls `resetToDefault(param)` and does not hard-code them.

The dedicated Bokeh setter accepted the exact documented strings `Circle`, `SoapBubble`, `Blade`, `Ring`, and `Anamorphic`; the Lightroom labels are Circle, Bubble, 5-Blade, Ring, and Anamorphic. The controlled change from Circle to `SoapBubble` was visually confirmed. The getter still returned Circle immediately after the successful setter and caught up later, proving that setter success cannot be used as selected-state authority. `toggleLensBlurDepthVisualization()` toggled the overlay, but the SDK provides no matching state getter. `selectTool("depth_refinement")` changed the tool without painting, and `getSelectedTool()` authoritatively changed from `loupe` to `depth_refinement`.

| Capability | Exact SDK surface | Healthy/enabled result | Disabled/unhealthy result | LRBridge status |
|---|---|---|---|---|
| Blur Amount | `getValue/getRange("LensBlurAmount")`; `setValue("LensBlurAmount", value)`; `resetToDefault("LensBlurAmount")` | Numeric R/W, range `0..100`; native reset observed 50 | Getter may be nil | Implemented, authoritative scalar and Lightroom-native Reset; nil disables editing |
| Cat Eye | `getValue/getRange("LensBlurCatEye")`; `setValue("LensBlurCatEye", value)`; `resetToDefault("LensBlurCatEye")` | Numeric R/W, range `0..100`; native reset observed 0 | Getter may be nil | Implemented, authoritative scalar and Lightroom-native Reset; nil disables editing |
| Boost | `getValue/getRange("LensBlurHighlightsBoost")`; `setValue("LensBlurHighlightsBoost", value)`; `resetToDefault("LensBlurHighlightsBoost")` | Numeric R/W, range `0..100`; native reset observed 50 | Getter may be nil | Implemented, authoritative scalar and Lightroom-native Reset; nil disables editing |
| Bokeh | `getSelectedLensBlurBokeh()` / `setLensBlurBokeh(value)` | Functional; getter may lag setter | Getter may be nil | Implemented with getter-only selection authority |
| Visualize Depth | `toggleLensBlurDepthVisualization()` | Functional action | No authoritative state getter | Implemented as a momentary action only |
| Brush Refinement tool | `selectTool("depth_refinement")` / `getSelectedTool()` | Tool selection and readback functional | Tool getter availability is reported independently | Implemented as tool selection only; no canvas input |
| Lens Blur Apply | `getValue/setValue("LensBlurActive", boolean)` | Explicit true/false setters proven; getter can return boolean true/false or nil | Getter may be nil; Develop settings can be absent/stale | WRITE-ONLY / INTERMITTENT READBACK; explicit Off/On remain usable with unknown presentation |
| Focus Range | `LensBlurFocalRange` | Scalar/compound/domain semantics not tested | Getter observed nil in failed state | Unknown; not implemented |
| Subject/point focus and refinement controls | No proven production contract | Not tested | Not tested | Not implemented |

**Conclusion:** Lightroom Classic 15.3 provides authoritative scalar and Bokeh feedback only while Lens Blur is healthy and readable. LRBridge implements three authoritative scalars with Lightroom-native Reset, explicit Apply Off/On with intermittent readback and an honest unknown state, authoritative-but-possibly-lagging Bokeh selection, momentary depth visualization, and authoritatively readable depth-refinement tool selection. It never fabricates Apply or scalar state during nil feedback and still does not expose Focus Range, subject/point focus, or refinement-brush settings. The original failed-state evidence remains part of the contract because it defines required unavailable behavior.
...
The 15.3 parameter catalog and documented `LrPhoto:getDevelopSettings()` members do not expose Lens Profile Setup, Make, Model, or Profile selection. They also expose no dedicated authoritative built-in-lens-profile status getter or reliable lens-profile inventory. LRBridge therefore does not render those selectors, a Make/Model/Lens Profile picker, or a built-in-profile status line. Slider availability alone is not used to infer that a built-in profile is applied. Specific lens profiles can be applied through ordinary configured Develop presets.
