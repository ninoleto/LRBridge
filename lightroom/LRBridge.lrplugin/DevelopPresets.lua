...
local function serverContextMatches(command)
    ...
    return jsonString(result, "activeModule") == command.expectedActiveModule and
...
local function validApplicationCommand(command)
    return type(command) == "table" and validId(command.operationId) and
        ...
        validInteger(command.presetAmount) and command.presetAmount >= 0 and command.presetAmount <= 200 and
        ...
        validInteger(command.expectedContextCounter) and validInteger(command.expectedDevelopCounter)
end
...
local function applyCapturedPhoto(command)
    ...
    local gateOk = LrTasks.pcall(function()
        catalog:withWriteAccessDo("LRBridge Develop Preset", function()
            ...
            capturedPhoto:applyDevelopPreset(preset, _PLUGIN, command.presetAmount, command.updateAISettings)
            sdkCompleted = true
        end)
    end)
